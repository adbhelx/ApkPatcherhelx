...
def Smali_Patch(decompile_dir, smali_folders, isAPKEditor, CA_Cert, isID, isPairip, Pairip_lib, isPKG, isPurchased, isSC, Skip_Patch, isUSB, isCoreX):
    ...
    patterns = [
        # ---------------- VPN Bypass ----------------
        (r'(const/4 [pv]\d+, 0x4[^>]*?invoke-\w+ \{[^\}]*\}, Landroid/net/NetworkCapabilities;->hasTransport\(I\)Z[^>]*?)move-result ([pv]\d+)', r'\1const/4 \2, 0x0', "Bypassed Vpn/Proxy Detection NetworkCapabilities hasTransport"),
        (r'(Ljava/net/NetworkInterface;->(?:isUp|isVirtual|isLoopback)\(\)Z[^>]*?)move-result ([pv]\d+)', r'\1const/4 \2, 0x0', "Bypassed Vpn/Proxy Detection NetworkInterface isUp"),
        (r'(const-string [pv]\d+, "(tun|tunl0|tun0|utun0|utun1|utun2|utun3|utun4|pptp|ppp|pp0|ppp0|p2p0|ccmni0|ipsec)"[^>]*?invoke-\w+ \{[^\}]*\}, L[^\(]+;->\S+\(Ljava/lang/CharSequence;\)Z[^>]*?)move-result ([pv]\d+)', r'\1const/4 \3, 0x0', "Bypassed Vpn/Proxy Detection NetworkInterface"),

        # ---------------- Mock Location & Update & Pkg Install Fixed ----------------
        (r'(invoke-virtual \{[^\}]*\}, Landroid/location/Location;->(?:isFromMockProvider|isMock)\(\)Z[^>]*?)move-result ([pv]\d+)', r'\1const/4 \2, 0x0', "Bypassed Mock Detection"),
    ...
    if isPKG:
        patterns.extend([
            # ---------------- Kill Process & Exit ----------------
            (r'invoke-static \{[^\}]*\}, (?:Ljava/lang/System;->exit|Landroid/os/Process;->killProcess)\(I\)V', 'nop', "Blocked System.exit & Process.killProcess"),

            # ---------------- Xposed Bypass & Frida Bypass ----------------
            (r'(const-string [pv]\d+, )"de.robv.android.xposed', r'\1"com.Fuck.U', "Bypassed Xposed Detection"),
            (r'const-string [pv]\d+, "(generic|goldfish)"[^>]*?invoke-static \{[^\}]*\}, Landroid/os/Build;->get(Device|Hardware)\(\)Ljava/lang/String;[^>]*?move-result-object [pv]\d+[^>]*?invoke-virtual \{[^\}]*\}, Ljava/lang/String;->contains\(Ljava/lang/CharSequence;\)Z[^>]*move-result ([pv]\d+)', r'const/4 \3, 0x0', "Bypassed Device detection"),
            (r'const-string [pv]\d+, "/data/local/tmp/(?:frida|frida-server)"[^>]*?invoke-static \{[^\}]*\}, Ljava/io/File;->exists\(\)Z[^>]*?move-result ([pv]\d+)', r'const/4 \1, 0x0', "Bypassed Frida Detection")
        ])
    ...
    if isSC:
        patterns.extend([
            # ---------------- Bypass SS ----------------
            (r'(const/16 [pv]\d+, 0x)200(0\s+(.line \d+\s+)*?invoke-virtual \{[^\}]*\}, Landroid/view/Window;->(?:add|set)Flags\(II\)V)', r'\1\2', "Bypassed Anti-Screen Detection <(add|set)Flags>"),
            (r'(invoke-static \{[^\}]*\}, L[^\(]+;->isSecuredNow\(Landroid/view/Window;\)Z\s+(.line \d+\s+)*?move-result [pv]\d+\s+(.line \d+\s+)*?const/16 ([pv]\d+),) 0x2000', r'\1 0x0', "Bypassed Anti-Screen Detection <isSecuredNow>"),
            (r'(iget [pv]\d+, [pv]\d+, Landroid/view/WindowManager\$LayoutParams;->flags:I\s+(.line \d+\s+)*?or-int/lit16 [pv]\d+, [pv]\d+,) 0x2000', r'\1 0x0', "Bypassed Anti-Screen Detection <flags:I>"),
            (r'(invoke-virtual \{([pv]\d+), ([pv]\d+)\}, Landroid/view/SurfaceView;->setSecure\(Z\)V)', r'const/4 \3, 0x0\n\n\t\1', "Bypassed Anti-Screen Detection <setSecure>")
        ])

    if isUSB:
        patterns.extend([
            # ---------------- Remove USB Debugging ----------------
            (r'(const-string [pv]\d+, "development_settings_enabled"[^>]*invoke-static \{[^\}]*\}, L[^\(]+;->getInt\([^\)]*Ljava\/lang\/String;I\)I[^>]*)move-result ([pv]\d+)', r'\1const/4 \2, 0x0', 'Remove USB Debugging <development_settings_enabled>'),
            (r'(const-string [pv]\d+, "adb_enabled"[^>]*invoke-static \{[^\}]*\}, L[^\(]+;->getInt\([^\(]*Ljava\/lang\/String;I\)I[^>]*)move-result ([pv]\d+)', r'\1const/4 \2, 0x0', 'Remove USB Debugging <adb_enabled>')
        ])
    ...
    if isPKG:
        counter = [1]
        patterns.extend([
            (rf'{P.Match_Regex}', lambda m: f'"com.Fuck.Me{(lambda x: counter.__setitem__(0, x+1) or x)(counter[0])}"', 'Spoof Package Detection in Dex'),
            (rf'{P.Match_Regex[1:-1]}', lambda m: f'"com.Fuck.Me{(lambda x: counter.__setitem__(0, x+1) or x)(counter[0])}"', 'Spoof Package Detection in Res')
        ])
...
def Check_CoreX(decompile_dir, isAPKEditor):
    lib_paths = C.os.path.join(decompile_dir, *(['root', 'lib'] if isAPKEditor else ['lib']))
    Lib_CoreX = []
