...
def Fix_Manifest(manifest_path, smali_folders, isPKG):
    ...
    patterns = [
        ...
        (r'\s+<[^>]*"(com.pairip.licensecheck)[^"]*"[^>]*/>' if isPC else r'\s+<[^>]*"com.(pairip.licensecheck|android.vending.CHECK_LICENSE)[^"]*"[^>]*/>', r'', 'CHECK_LICENSE')
        ]

    if isPKG:
        patterns.extend([(rf'{P.Menifest_Regex}', r'', 'Spoof Package Detection')])
